---
name: local-questionnaire
description: Create and run a reusable local browser questionnaire that autosaves structured answers, validates required and custom-detail responses, and lets Codex read the completed results to produce a config, specification, plan, or other artifact. Use when a task needs many user decisions, a structured interview, asynchronous form filling, a local questionnaire link, or when asking questions one turn at a time would be inefficient.
---

# Local Questionnaire

Use a local form when the user needs to answer several related questions. Keep the conversation for alignment and exceptions; use the form for collection.
...
4. Validate it:
   ...
   ```bash
   node <skill-dir>/scripts/questionnaire.mjs --config .questionnaires/<slug>/questionnaire.json --check
   ```

5. Check for an existing questionnaire server before starting another. Probe its `/api/config` endpoint to confirm it serves the same questionnaire. Otherwise run:

   ```bash
   node <skill-dir>/scripts/questionnaire.mjs --config .questionnaires/<slug>/questionnaire.json
   ```

6. Give the user the printed local URL. Open it when local GUI access is available. Keep the server running while the user fills the form.
7. When the user says they are finished, read `.questionnaires/<slug>/answers.json`. Continue only when `status` is `complete`; the server grants this status only after validating the full answer document.
...
Use the bundled zero-dependency Node server and static browser page. Do not add a database, frontend framework, form builder, authentication layer, or hosted service. This workflow is for local collaboration on the user's machine.
